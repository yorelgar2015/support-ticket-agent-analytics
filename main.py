...
if uploaded_file is not None:
    try:
        ...
        tickets = []
        has_id = any('id' in str(c).lower() for c in raw_df.columns)
        has_desc = any('desc' in str(c).lower() or 'text' in str(c).lower() or 'ticket' in str(c).lower() for c in raw_df.columns)

        if len(raw_df.columns) >= 2 and has_id and has_desc:
            id_col = next(c for c in raw_df.columns if 'id' in str(c).lower())
            desc_col = next(c for c in raw_df.columns if 'desc' in str(c).lower() or 'text' in str(c).lower() or 'ticket' in str(c).lower())
            for idx, row in raw_df.iterrows():
                tickets.append({
                    "id": str(row[id_col]).replace('.0', '').strip(),
                    "desc": str(row[desc_col]).strip().strip('"')
                })
        else:
            for idx, row in raw_df.iterrows():
                row_str = " ".join([str(val) for val in row.values if pd.notna(val)])
                tickets.append({"id": f"TICK-{idx + 1001}", "desc": row_str.strip()})

        st.subheader("📋 Ingested Dataset Preview")
        st.dataframe(pd.DataFrame(tickets).head(5), use_container_width=True)
        ...
        if st.button("🚀 Execute Multi-Agent Intelligence Pipeline"):
            ...
            for idx, t in enumerate(tickets):
                status_text.text(f"Processing Ticket {idx + 1} of {total_tickets} (ID: {t['id']})...")

                # Agent Pipeline Execution
                summary = run_agent(SUMMARIZER_SYSTEM_PROMPT, f"Ticket:\n{t['desc']}", selected_model, temp=0.1)
