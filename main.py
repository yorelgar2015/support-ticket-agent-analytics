...
selected_model = st.sidebar.selectbox("Select Model Engine", ["gpt-4o", "gpt-4o-mini"], index=0)
...
if uploaded_file is not None:
    try:
        ...
        tickets = []
        is_unstructured = len(raw_df.columns) > 0 and 'support_ticket_id,support_ticket_desc' in str(raw_df.columns[0])

        if is_unstructured:
            for idx, row in raw_df.iterrows():
                val_0 = str(row.iloc[0]) if pd.notna(row.iloc[0]) else ""
                if ',' in val_0:
                    # Split out the ID prefix from the first cell to get the description text start
                    _, desc_start = val_0.split(',', 1)
                    desc_start = desc_start.strip().strip('"')
                else:
                    desc_start = val_0.strip().strip('"')
                
                # Concatenate with columns 2, 3, etc. (index 1, 2, ...)
                other_cols = [str(val).strip() for val in row.iloc[1:] if pd.notna(val) and str(val).strip()]
                stitched_desc = " ".join([desc_start] + other_cols).strip().strip('"')
                
                tickets.append({
                    "support_ticket_id": str(idx + 1),
                    "support_ticket_desc": stitched_desc
                })
        else:
...
if 'results_df' in st.session_state:
    ...
    with t_c1:
        ...
        st.caption(f"**Summary Audit Score:** {float(sel_row['summary_score']):.2f} / 5.0 | **Reasoning:** {sel_row['summary_reasoning']}")

    with t_c2:
        st.subheader("Generated Customer Response Draft")
        st.warning(sel_row['generated_response'])
        st.caption(f"**Response Audit Score:** {float(sel_row['response_score']):.2f} / 5.0 | **Reasoning:** {sel_row['response_reasoning']}")
    ...
    # Preview Consolidated QA Report (First 2 Rows of All Columns, Index Hidden)
    st.subheader("📋 Consolidated QA Report Preview (First 2 Rows)")
    st.dataframe(df_res.head(2), hide_index=True, use_container_width=True)
    ...
    with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
        df_res.to_excel(writer, index=False, sheet_name='QA Report')
    ...
    st.download_button(
        label="📥 Download Consolidated QA Report (Excel)",
        data=excel_bytes,
        file_name="shopnest_consolidated_qa_report.xlsx",
